...
使用方法:
    WORKER_FILE_LIST=/tmp/files.txt WORKER_ID=0 blender -b --python render_ply_worker.py
"""
...
def timeout_handler(signum, frame):
    ...
    print(f"[Worker {os.environ.get('WORKER_ID', '?')}] TIMEOUT: Processing {state.current_file}")
    raise TimeoutError(f"Timeout processing {state.current_file}")
...
def process_ply_file(ply_path, output_dir, src_dir, timeout=120):
    """
    处理单个 PLY 文件，带超时保护
    timeout: 单文件处理超时（秒）
    """
...
if __name__ == "__main__":
    ...
    if not file_list_path:
        print("ERROR: WORKER_FILE_LIST environment variable not set")
        sys.exit(1)
    ...
    for i, ply_path in enumerate(ply_files):
        ...
        result, msg = process_ply_file(ply_path, output_dir, src_dir, timeout=120)
        ...
        if result == 'ok':
            ...
            timeout_count += 1
            flush_print(f"[Worker {worker_id}]   TIMEOUT: {msg}")
            # 超时是可重试的，记录到失败计数
            fail_count += 1
        else:
            # 错误
    ...
    flush_print(f"  Timeout: {timeout_count}")
